...
if (!SUPABASE_URL || !SUPABASE_SECRET_KEY) {
  ...
  describe('Empresas — Integration Tests (Supabase Real)', () => {
    ...
    it('EMP-INT-001: tenant A consegue acessar empresas via company_relationships', async () => {
    ...
    it('EMP-INT-002: tenant B não consegue acessar empresas do relacionamento A', async () => {
      const { data: relationshipsA } = await admin
        .from('company_relationships')
        .select('company_id')
        .eq('tenant_id', tenantA)
        .limit(1);

      if (relationshipsA && relationshipsA.length > 0) {
        const companyId = relationshipsA[0].company_id;
        const { data: relationshipsB } = await admin
          .from('company_relationships')
          .select('company_id')
        ...
        expect(relationshipsB?.length).toBe(0);
      }
    });
    ...
    it('EMP-INT-004: empresa global não possui tenant_id', async () => {
      ...
      if (data && data.length > 0) {
        expect(data[0]).not.toHaveProperty('tenant_id');
      }
    });
    ...
    it('EMP-INT-006: criação de relacionamento respeita tenant/RLS', async () => {
      ...
      if (existing && existing.length > 0) {
        ...
        const { data: rels } = await admin
          .from('company_relationships')
          .select('company_id, tenant_id, status')
          .eq('company_id', companyId)
          .eq('tenant_id', tenantA)
          .limit(1);
