...
import { MatchScoreBadge } from '@/features/candidato/components/candidate/MatchScoreBadge';
import { MatchBreakdown } from '@/features/candidato/components/candidate/MatchBreakdown';
...
export default function CandidateVagas() {
  const { matchResults, favoriteIds, isLoading, error, toggleFavorite } =
    useCandidate();
  ...
  const renderJobCard = (
    job: (typeof matchResults)[number]['job'],
    match: (typeof matchResults)[number]['match'],
    showExpand = true,
  ) => {
            ...
            <div className="min-w-0 flex-1">
              ...
              {showExpand && match && (
                <>
                  ...
                  <button
                    onClick={() =>
                      setExpandedMatchId(showMatchDetails ? null : job.id)
                    }
    ...
    return (
      <section className="space-y-4">
        ...
        <ul className="space-y-3">
          {jobs.map(({ job, match }) => renderJobCard(job, match, showExpand))}
        </ul>
      </section>
    );
  };
            ...
            {renderSection(
              ...
              search ? (
                <span className="text-muted-foreground text-xs">
                  {filteredAllJobs.length} resultado
                  {filteredAllJobs.length !== 1 ? 's' : ''}
                </span>
              ) : undefined,
            )}
          </div>
