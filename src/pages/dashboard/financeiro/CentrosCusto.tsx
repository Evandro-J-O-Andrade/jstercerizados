...
export default function FinanceiroCentrosCustoPage() {
  ...
  const [form, setForm] = useState<CostCenterCreateInput>({
    tenant_id: '',
    name: '',
    code: '',
    status: 'active',
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await costCenterRepository.findAll(currentTenantId);
      setItems(data);
    } catch (err) {
      setError(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!currentTenantId) return;
    try {
      await costCenterRepository.create({
        ...form,
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Organize gastos por centro de custo.
          </p>
        </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="Quando houver centros registrados, eles aparecerão aqui."
