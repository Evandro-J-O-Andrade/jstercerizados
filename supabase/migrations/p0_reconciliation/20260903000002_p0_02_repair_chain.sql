-- =============================================================================
-- P0-02 — REPAIR_CANDIDATE_CHAIN (corrige roles.code → roles.name)
-- =============================================================================
-- Data:    2026-09-03
-- Origem:  Blocker P0-02 do PREFLIGHT-20260902.md
-- Status:  AGUARDANDO OK EXPLÍCITO
-- =============================================================================
-- Problema: a função original (migration 02) usa WHERE code = p_role_code,
-- mas o `roles` real tem `name` e `scope`, NÃO `code`.
-- A função quebra com "column code does not exist".
--
-- Solução: CREATE OR REPLACE FUNCTION trocando `code` por `name`.
-- A assinatura do parâmetro p_role_code permanece (apenas o que muda
-- é o que está sendo procurado dentro do banco).
--
-- A role canônica 'candidate' tem name='candidate' e scope='tenant'.
-- A função agora a encontra corretamente.
--
-- Função é utilitária e MANUAL. Não auto-executa.
-- =============================================================================
-- Rollback:
...
BEGIN
  ...
  -- CORREÇÃO P0-02: roles usa `name`, não `code`
...
COMMENT ON FUNCTION public.repair_candidate_chain(uuid, uuid, text) IS
  'Utilitária MANUAL de reparo da cadeia auth.users → people → tenant_memberships → role_assignments → candidate. CORRIGIDA em P0-02: usa roles.name (não code). NÃO é executada automaticamente. Use SELECT repair_candidate_chain(''<person_id>''::uuid) para diagnosticar e reparar um registro.';
