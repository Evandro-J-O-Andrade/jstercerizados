...
create unique index if not exists companies_pending_cnpj_unique_idx
  on public.companies (cnpj)
  where status = 'pending' and cnpj is not null;

create or replace function public.register_company_interest(
  p_name text, p_cnpj text, p_email text, p_phone text,
...
declare
  ...
  v_name text:=btrim(p_name); v_cnpj text:=public.normalize_cnpj(p_cnpj);
...
begin
  ...
  values(v_tenant_id,v_name,v_cnpj,'pending',null,now(),now(),v_email,v_phone,jsonb_build_object('source','public_company_interest','contact_name',v_contact_name,'contact_phone',v_contact_phone,'contact_email',v_contact_email,'interest_message',v_message)) returning id into v_company_id;
  ...
  values(v_tenant_id,null,'company_interest_registered','company',v_company_id,jsonb_build_object('status','pending','source','public_company_interest'),jsonb_build_object('source','public_company_interest'),v_ip,v_user_agent,now());
