...
async function main() {
  const tables = ['tenants', 'people', 'tenant_memberships', 'companies', 'candidates', 'jobs', 'applications'];
  
  for (const table of tables) {
    const { data, error, count } = await c.from(table).select('*', { count: 'exact', head: true });
    console.log(`${table}: ${count ?? 0} rows, error=${error?.message}`);
  }
}
