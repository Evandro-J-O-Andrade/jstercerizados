 ...
 *
 * Executa:
 * 1. Cria usuÃ¡rio no Supabase Auth (idempotente)
 ...
 *
 * ðŸ” NÃƒO contÃ©m senha nem secret key em cÃ³digo.
 *   - SUPABASE_URL, SUPABASE_SECRET_KEY, ADMIN_EMAIL, ADMIN_PASSWORD
 *   - devem vir de variÃ¡veis de ambiente ou stdin
 *
 * Uso:
 *   SUPABASE_SECRET_KEY=xxxx ADMIN_EMAIL=evandro@... ADMIN_PASSWORD=xxxx npm run provision:admin
 *
 * ReferÃªncia: AGENTS.md
 *   "J&S Empregos LTDA" â€” NÃƒO alterar nome da empresa
 */
...
async function provision() {
  console.log('ðŸš€ Starting admin_master provision...');
  ...
  if (listError) {
    console.error('âŒ Failed to list users:', listError.message);
    process.exit(1);
  }
  ...
  if (foundUser) {
    ...
    console.log('âœ… User exists in Auth:', foundUser.id);
  } else {
    ...
    if (createError) {
      console.error('âŒ Failed to create user:', createError.message);
      process.exit(1);
    }
  ...
  if (personIdResponse.error) {
    console.error('âŒ Failed to fetch person:', personIdResponse.error.message);
    process.exit(1);
  }

  if (personIdResponse.data) {
    personId = personIdResponse.data.id;
    console.log('âœ… Person record exists:', personId);
  } else {
    console.log(
      'â„¹ï¸ Person record not found via trigger â€” creating manually...',
    );
    ...
    if (createPersonError || !newPerson) {
      console.error(
        'âŒ Failed to create person record:',
        createPersonError?.message || 'unknown error',
      );
      process.exit(1);
    }
    personId = newPerson.id;
    console.log('âœ… Person record created manually:', personId);
  }

  if (!personId) {
    console.error('âŒ Could not resolve person_id for admin user.');
    process.exit(1);
  }
  ...
  if (membershipError) {
    console.error(
      'âŒ Failed to create tenant membership:',
      membershipError.message,
    );
    process.exit(1);
  }
  console.log('âœ… Tenant membership verified (J&S Empregos LTDA)');

  // 5. Assign admin_master role (global â€” is_global = true)
  ...
  if (roleError) {
    console.error('âŒ Failed to assign admin_master role:', roleError.message);
    process.exit(1);
  }
  console.log('âœ… admin_master role assigned (global)');

  // Done
  console.log('');
  console.log('ðŸŽ‰ Provisionamento concluÃ­do:');
  ...
  console.log('   2. ApÃ³s login, verifique people + tenant_memberships');
  console.log(
...
provision().catch((err) => {
  console.error('âŒ Provision failed:', err.message || err);
  process.exit(1);
});
