...
let md = `# Portal RBAC Matrix\n\n`;
md += `Generated from Supabase RBAC dump.\n\n`;

md += `## Users → Roles\n\n`;
md += `| User | Role | Scope | Tenant |\n`;
md += `|------|------|-------|--------|\n`;
for (const pr of personRoles) {
  md += `| ${pr.personName} | ${pr.roleName} | ${pr.roleScope} | ${pr.tenantName} |\n`;
}

md += `\n## Roles → Permissions\n\n`;
for (const [roleId, permIds] of rolePermMap) {
  const role = roleMap.get(roleId);
  md += `### ${role?.name || roleId} (${role?.scope || 'unknown'})\n\n`;
  md += `| Permission | Resource | Action | Description |\n`;
  md += `|------------|----------|--------|-------------|\n`;
  for (const permId of permIds) {
    const p = permMap.get(permId);
    if (p) {
      md += `| ${p.id} | ${p.resource} | ${p.action} | ${p.description || ''} |\n`;
    }
  }
  md += `\n`;
}

fs.writeFileSync(path.join(process.cwd(), 'docs', 'PORTAL-RBAC-MATRIX.md'), md);
console.log('Generated docs/PORTAL-RBAC-MATRIX.md');
